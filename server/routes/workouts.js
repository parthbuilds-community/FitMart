...
router.get('/', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    logger.error(`Error fetching workout logs: ${err.stack || err.message}`);
...
router.post('/', verifyFirebaseToken, validateRequest(updateWorkoutLogSchema), async (req, res) => {
  try {
    ...
    logger.error(`Error saving workout log: ${err.stack || err.message}`);
...
router.delete('/:date', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    logger.error(`Error deleting workout log: ${err.stack || err.message}`);
