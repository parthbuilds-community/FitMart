  ...
  return (
    <span className="inline-flex items-center gap-0.5" aria-hidden>
      {Array.from({ length: full }).map((_, i) => (
        <svg key={i} className="w-3 h-3 text-stone-500" viewBox="0 0 24 24">
          <path d={path} fill="currentColor" />
        </svg>
      ))}
      {half ? (
        <svg className="w-3 h-3 text-stone-500" viewBox="0 0 24 24">
          <path d={path} fill="currentColor" style={{ opacity: 0.6 }} />
        </svg>
      ) : null}
      {Array.from({ length: empty }).map((_, i) => (
        <svg
          ...
          viewBox="0 0 24 24"
        >
          <path d={path} fill="currentColor" style={{ opacity: 0.28 }} />
        </svg>
      ))}
    </span>
  );
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`${API}/api/fitness-centers/nearby${q}`, {
          headers,
          credentials: "include",
        });
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {loading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="bg-white border border-stone-200 rounded-2xl p-4 animate-pulse h-36"
              />
            ))
          ) : centers.length === 0 ? (
            ...
            centers.map((c, i) => (
              ...
              >
                {i < 2 && (
                  <span className="absolute top-3 left-3 text-[10px] tracking-widest uppercase bg-stone-900 text-white px-2 py-1 rounded-full">
                    Near You
                  </span>
                )}
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-['DM_Serif_Display'] text-sm text-stone-900">
                        {c.name}
                      </h3>
                    ...
                    <div className="mt-1 text-xs text-stone-500 flex items-center gap-2">
                      <span className="text-[10px] uppercase text-stone-400 border border-stone-200 px-2 py-1 rounded-full">
                        {c.type}
                      </span>
