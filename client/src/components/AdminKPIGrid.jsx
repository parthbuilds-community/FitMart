...
// Extracted KPI cards from AdminDashboard.jsx for modularity
...
const SkeletonCard = () => (
  <div className="bg-stone-100 rounded-2xl animate-pulse h-28 sm:h-36" />
);

export default function AdminKPIGrid({ stats, loading }) {
  if (loading) {
    return (
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-5">
      <KPICard label="Total Revenue" value={fmt(stats.totalRevenue)} icon="₹" />
      <KPICard label="Total Orders" value={stats.totalOrders.toLocaleString()} icon="◎" />
      <KPICard label="Customers" value={stats.totalCustomers.toLocaleString()}
        icon={
      ...
      />
      <KPICard label="Low on Stock" value={stats.lowStockCount}
