        ...
        <button
          ...
          title={
            !c.eligibleForReminder
              ? `Customer must be inactive for 30+ days (currently ${c.daysSinceLastOrder} days)`
              : "Send reminder email to inactive customer"
          }
  ...
  const handleSendReminder = async (e, customerId) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/customers/${customerId}/send-reminder`,
        {
          method: "POST",
          headers,
          credentials: "include",
      ...
      if (!res.ok) {
        setReminderError((prev) => ({
          ...prev,
          [customerId]: data.error || "Failed to send",
        }));
              ...
              {["high-value", "returning", "new"].map((seg) => {
                ...
                return (
                  <div key={seg} className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full ${seg === "high-value" ? "bg-stone-900" : seg === "returning" ? "bg-stone-100 border border-stone-300" : "bg-stone-100"}`}
                    />
          ...
          <div className="bg-white border border-stone-200 rounded-2xl p-5 sm:p-7 hover:border-stone-300 hover:shadow-lg transition-all duration-300">
            ...
            <div className="space-y-3">
              ...
              {topBySpend.map((c, i) => {
                const pct = customers.length
                  ? Math.round(
                      (c.totalSpend / (topBySpend[0]?.totalSpend || 1)) * 100,
                    )
                  : 0;
                return (
