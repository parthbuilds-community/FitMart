    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300 ${bgClass}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 h-14 sm:h-16
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          <button
            ...
            className={`p-1.5 sm:hidden -ml-1 transition-colors ${textColor}`}
          >
        ...
        <div className="flex items-center gap-2">
          {/* GitHub Star — visible on all sizes, text label only on sm+ */}
          <a
            href={GITHUB_REPO}
            target="_blank"
            rel="noopener noreferrer"
            className={`flex items-center gap-1 sm:gap-1.5 text-xs px-2 sm:px-3 py-1.5 rounded-full
          ...
          >
            <GithubIcon className="w-3.5 h-3.5 hidden sm:block" />
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-4 px-4 flex flex-col gap-1">
          <button 
            onClick={() => { setMobileDrawerOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); }} 
