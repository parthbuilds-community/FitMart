...
const Empty = ({
  ...
  hint = "Try switching to a wider range",
}) => (
  <tr>
...
const ProductMobileCard = ({ p, index, productMap }) => {
  ...
  return (
    <div className="flex items-center gap-3 py-3.5 border-b border-stone-100 last:border-0">
      ...
      <span
        style={{ fontFamily: "'DM Serif Display', serif" }}
        className="text-lg text-stone-900 shrink-0"
      >
        {fmt(p.totalRevenue)}
      </span>
    </div>
...
export default function AdminReports() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/reports/sales?range=${range}`, {
          headers,
        });
  ...
  useEffect(() => {
    ...
    const ids = [
      ...new Set(productPerformance.map((p) => p.productId).filter(Boolean)),
    ];
  ...
  return (
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-5 lg:px-10 py-8 sm:py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 md:gap-5 mb-8 sm:mb-10">
          {[
            {
              label: "Total Revenue",
              value: loading ? null : fmt(summary.totalRevenue),
              icon: "₹",
            },
            {
              label: "Total Orders",
              value: loading ? null : summary.totalOrders,
              icon: "◎",
            },
            {
              label: "Avg Order Value",
              value: loading ? null : fmt(summary.avgOrderValue),
              icon: "─",
            },
          ...
          >
            ...
            >
              ...
              {!loading && productPerformance && (
                <p className="text-xs text-stone-400">
                  {productPerformance.length} products
                </p>
              )}
            </div>
      ...
      <footer className="border-t border-stone-200 bg-white mt-10 sm:mt-12">
        <div
          className="max-w-5xl mx-auto px-4 sm:px-5 lg:px-10 py-5 sm:py-6
                        flex justify-between items-center"
        >
          <span
            style={{ fontFamily: "'DM Serif Display', serif" }}
