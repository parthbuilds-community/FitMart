    ...
    <div className="min-h-screen bg-stone-50 font-['DM_Sans',sans-serif]">
          ...
          <div className={`fade-in d1 ${visible ? "show" : ""}`}>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
              <div>
                ...
                >
                  Mobility &{" "}
                  <em className="not-italic text-stone-400">Recovery</em>
                </h1>
                <p className="text-sm text-stone-300 max-w-lg leading-relaxed">
                  Flexibility-first programming, recovery protocols, and
                  rehabilitation plans — ideal for desk workers, athletes, and
                  anyone looking to move better.
                </p>
              </div>
              <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-10 py-10 sm:py-16 space-y-12 sm:space-y-16">
        <section>
          ...
          <div
            className={`fade-in d2 ${visible ? "show" : ""} grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5`}
          >
            {PLANS.map((plan, i) => (
              <div
                key={i}
              ...
              >
                <div className="flex items-center justify-between">
                  {plan.tag ? (
                    <span
                      className="text-[9px] tracking-[0.2em] uppercase rounded-full px-2.5 py-1
                ...
                <ul className="space-y-1.5 sm:space-y-2">
                  {plan.features.map((f, j) => (
                    <li
                      key={j}
                      className="text-xs flex items-start gap-2 text-stone-500"
                    >
                      <span className="text-stone-900 shrink-0 mt-0.5">✓</span>{" "}
                      {f}
                    </li>
                  ))}
                </ul>
              ...
              {[
                {
                  ...
                  desc: "Targeted myofascial release techniques restore tissue elasticity and reduce chronic tension patterns.",
                },
                {
                  icon: "⚡",
                  title: "Neural Mobility",
                  desc: "Nerve gliding exercises improve neural mobility, reducing pain and restoring full range of motion.",
                },
                {
                  icon: "✓",
                  title: "Movement Quality",
                  desc: "Corrective exercise progressions rewire motor patterns for pain-free, efficient movement.",
                },
              ].map((tip, i) => (
                <div key={i} className="bg-stone-100 rounded-2xl p-5 sm:p-7">
                  <span className="text-2xl mb-3 sm:mb-4 block">
                    {tip.icon}
                  </span>
