...
async function apiAddToCart(userId, productId, quantity) {
  const res = await apiFetch(`/api/cart/${userId}/add`, {
    auth: true,
    method: "POST",
    credentials: "include",
...
async function apiGetCart(userId) {
  const res = await apiFetch(`/api/cart/${userId}`, { auth: true, credentials: "include" });
...
export default function ProductPage() {
  ...
  const updateQty = async (id, delta) => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/cart/${user.uid}/${url}`, {
        auth: true,
        method: "POST",
        credentials: "include",
