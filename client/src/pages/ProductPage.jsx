...
function enrichCart(cartDoc, products) {
  return (cartDoc.items || []).map((it) => {
    const prod = products.find(
      (p) => Number(p.productId) === Number(it.productId)
    );
...
export default function ProductPage() {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.name} — FitMart`;
    }
  }, [product]);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const cartDoc = await apiAddToCart(
        user.uid,
        product.productId,
        quantity
      );
  ...
  const removeFromCart = async (id) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(
          errData.error || "Failed to remove"
        );
      }
  ...
  const updateQty = async (id, delta) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(
          errData.error || "Failed to update qty"
        );
      }
  ...
  if (loading) {
    return (
      ...
      >
        <div
          className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-10 py-10 sm:py-24
    ...
    return (
      ...
      >
        <div className="max-w-md mx-auto mt-20 sm:mt-32 text-center px-5">
          <p className="text-4xl text-stone-200 mb-4">
            ∅
          </p>
    ...
    <>
      ...
      >
        <style>{`
          ...
          .adding-pulse {
            animation: pdPulse .8s ease infinite;
          }
        ...
        >
          <div
            className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-10 py-3 sm:py-3.5
                       flex items-center gap-1.5 sm:gap-2 text-xs text-stone-400
        ...
        {/* ── Two-column main ── */}
        <div
          className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-10
        ...
        >
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 xl:gap-20">
            ...
            <div
              className={`pd-fade pd-d1 ${
                visible ? "in" : ""
              }`}
            >
              <div
                className="relative bg-stone-100 rounded-2xl overflow-hidden
                           aspect-square border border-stone-200 shadow-sm"
              >
                {product.image ? (
                  <img
                    ref={imgRef}
                    ...
                    /*
                     * Main product image is above the fold.
                     * Keep it eager instead of lazy loading.
                     * fetchPriority helps the browser load it early.
                     */
                    fetchPriority="high"
                    decoding="async"

                    onLoad={() => setImgLoaded(true)}
                    className={`pd-fade-img w-full h-full object-cover ${
                      imgLoaded ? "in" : ""
                    }`}
                  />
                ) : (
                ...
                >
                  {product.badge && (
                    <span
                      className="text-[10px] tracking-widest uppercase
                                 bg-stone-900 text-white px-3 py-1.5
                  ...
                  {discount && (
                    <span
                      className="text-[10px] font-medium bg-white text-stone-700
            ...
            <div className="flex flex-col" ref={stickyRef}>
              ...
              >
                <span
                  className="text-[10px] tracking-[0.2em] uppercase text-stone-400
                             border border-stone-200 px-3 py-1.5 rounded-full"
                >
                  {product.brand}
                </span>
              ...
              <h1
                ...
                className={`pd-fade pd-d2 ${
                  visible ? "in" : ""
                } text-3xl sm:text-4xl lg:text-5xl text-stone-900
                   leading-[1.1] tracking-tight mb-3 sm:mb-4`}
              >
                {product.name}
              </h1>
              ...
              {/* Price card */}
              <div
                className={`pd-fade pd-d3 ${
                  visible ? "in" : ""
                } bg-stone-50 border border-stone-200 rounded-2xl
                    ...
                    product.originalPrice > product.price && (
                      <span
                        className="text-base sm:text-lg text-stone-400
                                   line-through leading-none mb-0.5"
                      >
              ...
              >
                ...
                <div
                  className="flex items-center border border-stone-200
                             rounded-full w-fit overflow-hidden"
                >
                  <button
                    onClick={() =>
                    ...
                    className="qty-btn w-11 h-11 flex items-center
              ...
              >
                <button
                  ...
                  disabled={busy}
                  className={`flex-1 text-sm py-4 rounded-full font-medium
                              transition-all flex items-center justify-center
                              gap-2 disabled:cursor-not-allowed min-h-13
                              active:scale-[0.98]
                              ${
                                added
                ...
                <button
                  ...
                  disabled={busy}
                  className={`border border-stone-300 text-stone-700 text-sm
                              px-6 py-4 rounded-full transition-all
                              flex items-center justify-center gap-2 min-h-13
                              active:scale-[0.98]
                              ${
                                buyingNow
        ...
        <div className="border-t border-stone-200 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-10">
            ...
            <div
              className="flex gap-4 sm:gap-8 border-b border-stone-100
                         overflow-x-auto scrollbar-none -mx-4 sm:mx-0
                         px-4 sm:px-0"
            >
              {[
                {
              ...
              {activeTab === "details" && (
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-stone-600 leading-relaxed">
                    {product.name} by {product.brand} is a premium{" "}
                    {product.category?.toLowerCase()} product trusted by
                    fitness enthusiasts across Mumbai.
                    {product.badge
                      ? ` Rated as "${product.badge}" by our community.`
                      : ""}
                    {" "}
                    Sourced directly from the manufacturer and verified
                    for authenticity.
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:gap-4 pt-4">
                    {[
                      ...
                      <div
                        key={label}
                        className="bg-stone-50 rounded-xl px-3
                                   sm:px-4 py-3"
                      >
                        <p
                          className="text-[10px] tracking-[0.15em]
                                     uppercase text-stone-400 mb-1"
                        >
                          {label}
                        </p>
                  ...
                  {[
                    ...
                    <div
                      ...
                      className="flex items-center justify-between
                                 py-3 border-b border-stone-100
                                 last:border-0 gap-4"
                    >
                      <span
                        className="text-xs text-stone-500 uppercase
                  ...
                  {[
                    {
                      ...
                      title: "Fast Delivery",
                      body:
                        "Orders dispatched within 24 hours. Delivery within 2–5 business days across MMR.",
                    },
                    {
                      icon: "✓",
                      title: "Free Shipping",
                      body:
                        "Free shipping on all orders above ₹499. Standard charges of ₹49 apply below that threshold.",
                    },
                    {
                      icon: "◎",
                      title: "Easy Returns",
                      body: `${
                        product.category === "Wearables"
                          ? "15"
                          : "30"
                      }-day hassle-free returns. Item must be unused and in original packaging.`,
                    },
                  ].map(({ icon, title, body }) => (
                    <div
                      key={title}
                      className="flex gap-3 sm:gap-4 p-4 sm:p-5
                ...
                {related.map((rel) => {
                  ...
                  return (
                    <div
                      key={rel.productId}
                      onClick={() =>
                        navigate(`/product/${rel.productId}`)
                      }
                      className="related-card bg-white border border-stone-200
                                 rounded-2xl overflow-hidden cursor-pointer
                                 hover:border-stone-300 hover:shadow-lg
                                 transition-all duration-300"
                    >
                      <div
                        className="relative bg-stone-100 aspect-square
                                   overflow-hidden"
                      >
                        {rel.image ? (
                          <img
                            ...
                            /*
                             * Related products are below the main
                             * product content, so native lazy loading
                             * prevents unnecessary initial downloads.
                             */
                            loading="lazy"
                            decoding="async"
                          ...
                          <div
                            className="w-full h-full flex items-center
                                       justify-center text-4xl opacity-20"
                          >
                            {rel.category === "Nutrition"
                              ? "🧴"
                              : rel.category === "Wearables"
                              ? "⌚"
                              : "🏋️"}
                          </div>
                        )}
                        ...
                        {rel.badge && (
                          <span
                            className="absolute top-2 left-2 text-[9px]
                                       tracking-widest uppercase bg-stone-900
                        ...
                        {rd && (
                          <span
                            className="absolute top-2 right-2 text-[9px]
                                       ...
                                       border border-stone-200 px-2 py-1
                                       rounded-full"
                          >
                            −{rd}%
                          </span>
        ...
        >
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              ...
              {cartCount > 0 && (
                <span
                  className="absolute -top-0.5 -right-0.5 bg-stone-900
            ...
            <button
              onClick={() => navigate("/home")}
              className="border border-stone-200 text-stone-600 text-xs
                         px-3 sm:px-5 py-2 rounded-full
                         hover:bg-stone-900 hover:text-white
                         hover:border-stone-900 transition-all min-h-9"
            >
