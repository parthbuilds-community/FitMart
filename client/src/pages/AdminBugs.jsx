      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-5 lg:px-10 py-8 sm:py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 md:gap-5 mb-8 sm:mb-10">
          {[
            ...
            >
              <p className="text-xs tracking-[0.2em] uppercase text-stone-400 mb-4 sm:mb-5">
                {label}
              </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 sm:mb-10">
          <div className="bg-white border border-stone-200 rounded-2xl p-5 sm:p-7 hover:border-stone-300 hover:shadow-lg transition-all duration-300">
            ...
            <h3
              style={{ fontFamily: "'DM Serif Display', serif" }}
              className="text-lg text-stone-900 mb-4"
            >
              Status distribution
            </h3>
            <div className="space-y-3">
              {["open", "in-progress", "resolved"].map((status) => {
                ...
                const pct = bugs.length
                  ? Math.round((count / bugs.length) * 100)
                  : 0;
                return (
                  <div key={status} className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full ${status === "open" ? "bg-stone-900" : status === "in-progress" ? "bg-stone-100 border border-stone-300" : "bg-stone-100"}`}
                    />
          ...
          <div className="bg-white border border-stone-200 rounded-2xl p-5 sm:p-7 hover:border-stone-300 hover:shadow-lg transition-all duration-300">
            ...
            <div className="space-y-3">
              {bugs.length === 0 && (
                <p className="text-xs text-stone-400">No reports yet</p>
              )}
        ...
        >
          ...
          >
            ...
            {!loadingBugs && (
              <p className="text-xs text-stone-400">{bugs.length} reports</p>
            )}
          </div>
          ...
          <div className="md:hidden px-4 py-2">
            {loadingBugs &&
              [...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="h-14 bg-stone-100 rounded-xl animate-pulse mb-3"
                />
              ))}
              ...
              <tbody className="divide-y divide-stone-100">
                ...
                {!loadingBugs &&
                  sortedBugs.map((bug, index) => (
                    ...
                    >
                      <td className="px-6 py-5 text-stone-300 text-xs">
                        {index + 1}
                      </td>
                      ...
                      <td className="px-6 py-5 text-center">
                        {bug.screenshot ? (
                          <a
                            href={`${import.meta.env.VITE_API_URL || "http://"}${bug.screenshot}`}
                            target="_blank"
                          ...
                          >
                            <img
                              src={`${import.meta.env.VITE_API_URL || "http://"}${bug.screenshot}`}
                              alt="Screenshot"
